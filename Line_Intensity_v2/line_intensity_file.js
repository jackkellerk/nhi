 ...
 */
function LI_hideAll(windowContainer) {
    LI_state = "nuetral";
    ...
    for (var i = 0; i < bcLength; i++) {
        ...
        windowContainer.removeChild(LI_buttonCommands[i]);
    }

    windowContainer.removeChild(LI_mainText);
    windowContainer.removeChild(LI_cancel_button);

    windowContainer.removeChild(LI_backgroundImage);
    windowContainer.removeListener("pointerdown", LI_drawPoint);
}
...
function LI_endDraw() {
    ...
    windowContainer.interactive = false;
}
...
function LI_onDragPointStart(event) {
    if (LI_state == "edit") {
        ...
        LI_pointContainer[this.name].owner.resetImage(windowContainer);
        LI_pointContainer[this.name].resetImage(windowContainer);
        this.dragging = true;
...
function LI_onDragPointEnd() {
    if (LI_state == "edit") {
        ...
        LI_pointContainer[this.name].owner.displayDetails(windowContainer);
...
function LI_onDragLineEnd() {
    if (LI_state == "edit") {
        ...
        LI_lineContainer[this.name].resetImage(windowContainer);
        LI_lineContainer[this.name].startPoint.image.alpha = 1;
        LI_lineContainer[this.name].startPoint.resetImage(windowContainer);
        LI_lineContainer[this.name].endPoint.image.alpha = 1;
        LI_lineContainer[this.name].endPoint.resetImage(windowContainer);
        LI_lineContainer[this.name].displayDetails(windowContainer);
        this.dragging = false;
        updateGraph(LI_graphs, LI_currentLine, LI_boundary_tlx, LI_boundary_tly);
        windowContainer.addChild(LI_graphs);
