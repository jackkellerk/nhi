...
function startPreLogin() {
  ...
  //right now these are just images found on google
  var imageArray = [
    "Images/login_background2.jpg",
    "Images/login_background3.jpg",
    "Images/login_background4.jpg",
    "Images/login_background5.jpg",
    "Images/login_background6.jpg",
    "Images/login_background7.jpg",
    "Images/login_background8.jpg",
    "Images/login_background9.jpg",
  ];

  //selects a random image
  var imageIndex = Math.floor(Math.random() * imageArray.length);
  //uses selected image as backdrop
  login_backgroundImage = new PIXI.Sprite.from(imageArray[imageIndex]);

  if (app.screen.width > app.screen.height) {
    login_backgroundImage.height = app.screen.height;
    login_backgroundImage.width = app.screen.height * (1024 / 680);
    login_backgroundImage.x =
      app.screen.width / 2 - login_backgroundImage.width / 2;
    ...
    login_backgroundImage.y =
      app.screen.height / 2 - login_backgroundImage.height / 2;
  }
  login_backgroundImage.on("pointerdown", touchScreen);
  login_backgroundImage.alpha = 1.0;

  display_text1.fontSize = login_backgroundImage.height * 0.08;
  display_text1.wordWrapWidth = login_backgroundImage.width;
  login_style.fontSize = login_backgroundImage.height * 0.06;
  signUp_style.fontSize = login_backgroundImage.height * 0.04;
  ...
  preDisplayAuthor.y =
    preDisplayQuote.y + preDisplayQuote.height + preDisplayAuthor.height;
  ...
  loginButtonImage = new PIXI.Sprite.from(
    "Images/Sources/Lehigh/preloginbutton.png"
  );
  ...
  var _text = login_backgroundImage.height * 0.025;
  if (_text < 1) {
    _text = 1;
  }
  var _size = "" + _text + "pt";
  userTextBox = new PIXI.TextInput({
    ...
    box: generateTextLine(
      login_hex.x - (pl_radius * 2 * 0.58) / 2,
      login_hex.y - _text * 2,
      pl_radius * 2 * 0.58,
      3,
      1
  ...
  userTextBox.x = login_hex.x - (pl_radius * 2 * 0.58) / 2;
  ...
  passwordTextBox.x = login_hex.x - (pl_radius * 2 * 0.58) / 2;
  ...
  signUpText.y = loginText.y + login_backgroundImage.height * 0.1;
  ...
  login_UI.alpha = 1;
  scaleTransform(0.1, 0.1, login_UI, 1);
  positionTransform(
    app.screen.width - login_UI.width * 0.1 * 1.2,
    app.screen.height - login_UI.height * 0.1 * 1.2,
    login_UI,
    1
  );
  ...
  SU_signUpText.y = signup_hex.y + login_backgroundImage.height * 0.22;
...
function generateTextLine(x, y, w, lineWidth, type) {
  ...
  line
    ...
    .lineTo(x + w, y);
  if (type == 1) {
    Inner_Login_UI.addChild(line);
  } else if (type == 2) {
    signUp_UI.addChild(line);
  }
}
...
function hideLogin(event) {
  ...
  //Move Hexagon to front of the screen
  scaleTransform(0.1, 0.1, login_UI, 15);
  positionTransform(
    app.screen.width - login_UI.width * 0.1 * 1.2,
    app.screen.height - login_UI.height * 0.1 * 1.2,
    login_UI,
...
function CheckAFK(flag) {
  ...
  let TimerIncrease = () => {
    //We are declaring the animation function here
    if (count >= limit) {
      hideLogin();
